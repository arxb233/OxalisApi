...
{
    public partial class TgBotMessages(TgBotClassRespose tb, TelegramBotClient _bot, Message msg) : IDisposable
    {
        public StringBuilder stringBuilder = new();
        public Message StartMessage = new();
        public Message EndMessage = new();
        public LinuxSshHelper? sshHelper;
    ...
    ;
        ...
        private async Task OutPutMessage(LinuxSshHelper ssh)
        {
        ...
        private async Task<bool> Wallet()
        {
            #region Wallet
            await SendProcess($"任务开始执行....");
            await SendProcess($"1.正在查询AutodL钱包信息....");
            var Wallet = await AutoDLClass.Wallet(tb.AutoDL.Authorization);
            if (Wallet <= 1.5) { await SendProcess($"1.AutoDL余额不足1.5元,请保证余额充足再使用！"); return false; }
            await SendProcess($"2.当前AutoDL账户余额为{Wallet}元,正在查询AutoDL设备信息....");
            #endregion
            return true;
        }
        private async Task<bool> GPU()
        {
            #region GPU
            var Check = await AutoDLClass.Check(tb.AutoDL.Authorization, tb.AutoDL.Instance_uuid);
            if (Check == -1) { await SendProcess($"当前AutoDL实列已存在,请稍后使用！"); return false; }
            if (Check == 0) { await SendProcess($"当前AutoDL实列没有可用GPU,请稍后使用！"); return false; }
            await SendProcess($"3.当前AutoDL实列可用GPU为{Check}个,满足运行条件！");
            #endregion
            return true;
        }
        private async Task<(bool, Stream)> Download(string MatchUrl, string detail)
        {
            #region Download
            await SendProcess($"4.视频链接获取成功，正在下载视频，请耐心等待....");
            using var video = await VideoDownClass.DownLoad(tb.Video, MatchUrl, detail);
            if (video == Stream.Null || video.Length <= 333) { await SendProcess("视频获取失败！"); return (false, Stream.Null); }
            return (true, video);
            #endregion
        }
        private async Task<bool> Power_on()
        {
            #region Power_on
            ...
            if (!Open) { await SendProcess($"当前AutoDL实列开机失败,请联系管理员！"); return false; }
            do { if (await AutoDLClass.Check(tb.AutoDL.Authorization, tb.AutoDL.Instance_uuid) == -1) { break; }; await Task.Delay(5000); } while (true);
            #endregion
            return true;
        }
        private async Task<bool> SSH()
        {
            #region SSH
            await SendProcess($"6.开机成功,正在进行远程链接....");
            sshHelper = new LinuxSshHelper(tb.AutoDL.Host, tb.AutoDL.Port, tb.AutoDL.Username, tb.AutoDL.Password);
            sshHelper.Connect(); sshHelper.OpenPort(tb.ComfyUI.Port);
            #endregion
            return true;
        }
        private async Task<bool> UploadVideo(Stream video)
        {
        ...
        private async Task<bool> ComfyUI()
        {
            if (sshHelper is not null)
            {
                ...
                if (sshHelper.FileExists(tb.ComfyUI.OutputPath[0]))
                {
                    ...
                    await OutPutMessage(sshHelper); return false;
                }
                #region ComfyUI
                ...
                if (await comfyUIClass.Prompt() is (bool, string) PromptResult && !PromptResult.Item1) { await SendProcess(PromptResult.Item2); await AutoDLClose(); return false; }
                #endregion
            }
            return true;
        }
